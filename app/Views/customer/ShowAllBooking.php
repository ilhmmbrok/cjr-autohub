...
<div class="w-full bg-white px-4 sm:px-6 lg:px-8 py-8 animate-fadeInUp">
    ...
    <?php if (empty($bookings)): ?>
        <div class="group relative rounded-2xl border border-zinc-200 bg-white p-8 overflow-hidden hover:border-zinc-300 transition-all duration-300  shadow-xs hover:shadow-md">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <?php foreach ($bookings as $booking): ?>
                ...
                <div class="group relative bg-white border border-zinc-200 rounded-2xl p-5 hover:shadow-lg hover:border-zinc-300 transition-all duration-300 hover-lift">
                    ...
                    <!-- Main Info -->
                    <div class="mb-4">
                        <h3 class="text-sm font-bold text-zinc-800 truncate "><?= htmlspecialchars($booking['model_year']) ?></h3>
                    ...
                    <div class="flex items-center gap-4 mb-6 py-3 border-y border-zinc-50 group-hover:border-zinc-100 transition-colors">
                        <div class="flex flex-col">
                            ...
                            <span class="text-sm font-semibold text-zinc-800"><?= htmlspecialchars(date('d M Y', strtotime($booking['booking_date']))) ?></span>
                        </div>
                        <div class="w-px h-6 bg-zinc-100"></div>
                        <div class="flex flex-col">
                            <span class="text-[10px] text-zinc-400 uppercase font-medium">Jam Kedatangan</span>
                            <span class="text-sm font-semibold text-zinc-800"><?= htmlspecialchars(date('H:i', strtotime($booking['checkin_time']))) ?></span>
                        </div>
                    </div>
