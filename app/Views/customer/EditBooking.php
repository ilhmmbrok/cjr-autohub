                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="address" id="address" rows="3" placeholder="Alamat Lengkap" required
                                class="w-full px-3 py-2 rounded-md border text-sm border-zinc-300 bg-white focus:outline-none focus:ring-2 focus:ring-zinc-950/10 focus:border-zinc-400 transition-all resize-none"><?= htmlspecialchars($booking['address'] ?? '') ?></textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select id="checkin_time" name="checkin_time" required
                                    ...
                                    <option value="" disabled <?= empty($booking['checkin_time']) ? 'selected' : '' ?>>Pilih jam kedatangan</option>
                                    <?php for ($hour = 0; $hour < 24; $hour++): ?>
                                        <?php $value = sprintf('%02d:00', $hour); ?>
                                        <option value="<?= $value ?>" <?= (($booking['checkin_time'] ?? '') === $value) ? 'selected' : '' ?>>
                                            <?= $value ?>
                                        </option>
                                    <?php endfor; ?>
                                </select>
                    ...
                    <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select name="vehicle_type" id="vehicle_type" required
                                        class="w-full appearance-none px-3 py-2 rounded-md text-sm border border-zinc-300 bg-white focus:outline-none focus:ring-2 focus:ring-zinc-950/10 focus:border-zinc-400 transition-all cursor-pointer">
                                        <option value="motor" <?= (($booking['vehicle_type'] ?? '') === 'motor') ? 'selected' : '' ?>>Motor</option>
                                        <option value="mobil" <?= (($booking['vehicle_type'] ?? '') === 'mobil') ? 'selected' : '' ?>>Mobil</option>
                                    </select>
                                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-zinc-400">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        ...
                        <div>
                            ...
                            <textarea name="customer_complaint" id="customer_complaint" rows="5"
                                ...
                                class="w-full h-20 md:h-41 px-3 py-2 rounded-md border text-sm border-zinc-300 bg-white focus:outline-none focus:ring-2 focus:ring-zinc-950/10 focus:border-zinc-400 transition-all resize-none"><?= htmlspecialchars($booking['customer_complaint'] ?? '') ?></textarea>
                        </div>
                    </div>
                </div>
