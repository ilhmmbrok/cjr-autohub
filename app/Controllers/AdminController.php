...
class AdminController extends Controller
{
    ...
    public function updateProfile(): void
    {
        ...
        if (empty($fullname) || empty($email)) {
            $this->abort('error', 'Nama dan email wajib diisi.', '/admin/profile');
        }

        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $this->abort('error', 'Format email tidak valid.', '/admin/profile');
        }

        // Cek apakah email sudah dipakai user lain
        $existing = $this->userModel->findByEmail($email);
        if ($existing && (int) $existing['id'] !== (int) $user['id']) {
            $this->abort('error', 'Email sudah digunakan oleh akun lain.', '/admin/profile');
        }

        // Validasi format nomor telepon (opsional)
        if (!empty($phone) && !preg_match('/^[0-9+\-\s()]{8,20}$/', $phone)) {
            $this->abort('error', 'Format nomor telepon tidak valid.', '/admin/profile');
        }
        ...
        $this->abort('success', 'Profil berhasil diperbarui.', '/admin/profile');
    }
    ...
    public function updatePassword(): void
    {
        ...
        if (empty($oldPassword) || empty($newPassword) || empty($confirmPassword)) {
            $this->abort('error', 'Semua field password wajib diisi.', '/admin/profile');
        }

        if (strlen($newPassword) < 8) {
            $this->abort('error', 'Password baru minimal 8 karakter.', '/admin/profile');
        }

        if ($newPassword !== $confirmPassword) {
            $this->abort('error', 'Konfirmasi password tidak cocok.', '/admin/profile');
        }

        // Verifikasi password lama
        $fullUser = $this->userModel->findById($user['id']);
        if (!$fullUser || !password_verify($oldPassword, $fullUser['password'])) {
            $this->abort('error', 'Password lama tidak benar.', '/admin/profile');
        }

        $this->userModel->updatePassword($user['id'], password_hash($newPassword, PASSWORD_DEFAULT));

        $this->abort('success', 'Password berhasil diubah.', '/admin/profile');
    }
}
